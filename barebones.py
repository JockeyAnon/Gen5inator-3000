...
mean = np.mean(np.split(np.mean(np.split(np.array(rawnorm),2), axis = 0),2),axis=0)
std = np.std(np.split(np.std(np.split(np.array(rawnorm),2), axis = 0),2),axis=0)
...
foldchange = []
foldchangemeans = [[np.divide(j,i[-1]) for j in i]for i in mean]
